...
procedure TDemoEventHandler.OnStateChange(Sender: TObject; NewState: TUpdateState;
  const StatusMessage: string);
begin
  WriteLn(Format('[%s] %s', [
...
procedure TDemoEventHandler.OnDownloadProgress(Sender: TObject; BytesReceived, TotalBytes: Int64;
  var Cancel: Boolean);
var
  Percent: Integer;
begin
...
begin
  ...
  // Create event handler and update engine
  EventHandler := TDemoEventHandler.Create;
  Engine := TUpdateEngine.Create(Config);
  try
    Engine.OnStateChange := EventHandler.OnStateChange;
    Engine.OnDownloadProgress := EventHandler.OnDownloadProgress;
    ...
    begin
      ...
      Readln(UserInput);
      if (Length(UserInput) > 0) and (UpCase(UserInput[1]) = 'Y') then
      begin
        WriteLn;
